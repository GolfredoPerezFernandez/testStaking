...
const sendOptions2 = {
	...
	awaitReceipt: true,
	params: {
		_stakerAddress:user?.get("ethAddress"),
	},
  };
  const res22: any = await Moralis.executeFunction(sendOptions2);
  if(res22){
...
tokenIds=[...tokenIds,res3[i][1]];
  ...
  const [tvl3,setTVL3]=useState("0")
  const [tvl4,setTVL4]=useState("0")
  const [tvl5,setTVL5]=useState("0")
  const [tvl,setTVL]=useState("0")
  const [nftV1TVL,setNftV1TVL]=useState("0")
  const [nftV2TVL,setNftV2TVL]=useState("0")
  const [nftV1,setNftV1]=useState("0")
  const [nftV2,setNftV2]=useState("0")
...
if(res.status === 200) { 
 listFiltered2=res.data.result.filter((item:any)=>item.tokenID!==undefined&&item.contractAddress==='0xFdfDab3Df0fFE67b735b7B78acf3356913bbcEe7'.toLowerCase()) 
}})	
 ...
 for(let i=0 ; i < parseInt(lista[0].balance) ; i++){
  ...
  const handleUpdateStakingV1 = async () => {
    try {
      ...
      setPendingStakingV1(Moralis.Units.FromWei(parseInt(pen)));
  ...
  const handleUpdateStakingV1New20 = async () => {
	try {
		...
		const pen2 = await contract.connect(signer).calculateRewards(user?.get('ethAddress'));
...
if(res.status === 200) { 
 ...
 const lista = await res.data.result.filter( (item:any) => item.contractAddress.toLowerCase()==="0xfdfdab3df0ffe67b735b7b78acf3356913bbcee7".toLowerCase())
 if(lista.length!==0){
  ...
  const handleUpdateStakingV2New = async () => {
    try {
      ...
      const transaction = await contract.connect(signer).balances(user?.get('ethAddress'));
	    setPendingStakingV2(Moralis.Units.FromWei(parseFloat(pen[2]).toString()));
 ...
 <Stack style={{ paddingLeft:props.width<1000?"0px":"0px",flexDirection:props.width<1000?"column":"column", marginTop: props.width<1000?0:40,marginBottom: 0, width: '100%', justifyContent: props.width<1000?"center":'center',gap: props.width<1000?70:100, alignItems: props.width<1000?"center":'center' }}>
    ...
    {props.width<1000?<VStack style={{width:'80%'}}>
	   ...
	   <HStack style={{width:props.width < 1000 ?'80%':'80%'}}>
		   <video muted loop  autoPlay src={"https://bafybeia7qmxiwhnzmbbc7oevuwcyr6x2qldb4oaxiqzyaktjx63mgbilzu.ipfs.w3s.link/Main_Render_2.mp4"} />
		   </HStack>	
		   </VStack>:
...
<HStack style={{width:props.width < 1000 ?'30%':'80%'}}>
	<video muted autoPlay src={"https://bafybeia7qmxiwhnzmbbc7oevuwcyr6x2qldb4oaxiqzyaktjx63mgbilzu.ipfs.w3s.link/Main_Render_2.mp4"} />
	</HStack>	
	</HStack>}
{props.width < 1000 ? (
...
<Heading   fontSize="2xl" marginBottom={4} textAlign={'center'}>
   ...
   <HStack mb={2}  alignItems={"center"} justifyContent={"center"}>
	...
	<Text  fontSize="sm" width={40} textAlign={'center'}>
	{pendingStakingV1.toString().concat(' Cooties')}
	</Text>
					...
					/>    
					 <Button
                      ...
                      <Box style={{ height: 10 }} />
						<Button
                          disabled={user ? false : true}
                          onClick={handleUpgrade2}
                          isFullWidth={true}
						  color="red"
                          text="Withdraw Old Staking"
                          theme="colored"
                        />
                         <Box style={{ height: 10 }} />
...
<Heading   fontSize="2xl" marginBottom={4} textAlign={'center'}>
   ...
   <HStack mb={2}  alignItems={"center"} justifyContent={"center"}>
	...
	<Text  fontSize="sm" width={40} textAlign={'center'}>
	{pendingStakingV2.toString().concat(' Cooties')}
	</Text>
